...
export function buildSubagentResultIntercomPayload(input: GroupedResultIntercomMessageInput): SubagentResultIntercomPayload {
	const withNestedChildren = attachNestedChildrenToResultChildren(input.runId, input.children, input.nestedChildren);
	const children = withNestedChildren.map((child) => ({
		...child,
