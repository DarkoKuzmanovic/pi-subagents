...
import { type ExtensionAPI, type ExtensionContext, type ToolDefinition, keyText } from "@earendil-works/pi-coding-agent";
...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	setInlineReadMaxBytes(config.inlineReadMaxBytes);
	...
	pi.registerMessageRenderer<SubagentNotifyDetails>("subagent-notify", (message, options, theme) => {
		...
		if (!options.expanded && trimmedPreview.includes("\n")) {
			text += `
  ${theme.fg("dim", `${keyText("app.tools.expand")} full notification`)}`;
		}
...
• Optional context: { context: "fresh" | "fork" | "lineage" } (default: if any requested agent has defaultContext: "fork", the whole invocation uses fork; otherwise if any has defaultContext: "lineage", it uses lineage; otherwise "fresh"; inspect agent defaults via { action: "list" })
