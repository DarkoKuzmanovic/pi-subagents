...
function foregroundStyleWidgetStepLines(
	...
	if (step.status === "running") {
		if (!expanded) lines.push(`    ${theme.fg("accent", `Press ${keyText("app.tools.expand")} for live detail`)}`);
...
function compactSingleWidgetLines(job: AsyncJobState, theme: Theme, width: number): string[] {
	...
	for (const [index, step] of job.steps.entries()) {
		...
		lines.push(`  ${widgetStepGlyph(step.status, theme)} ${itemTitle} ${index + 1}/${total}: ${themeBold(theme, step.agent)}${renderModelTag(step.model, theme)} ${theme.fg("dim", "·")} ${status}${activitySuffix}${stepStats ? ` ${theme.fg("dim", "·")} ${stepStats}` : ""}`);
	}
	if (job.steps.some((step) => step.status === "running")) lines.push(theme.fg("accent", `  Press ${keyText("app.tools.expand")} for live detail`));
...
function fitWidgetLineBudget(lines: string[], theme: Theme, width: number, expanded: boolean): string[] {
	...
	const hint = expanded
		...
		: `… ${hiddenCount} lines hidden · ${keyText("app.tools.expand")} expands`;
...
function renderSingleCompact(d: Details, r: Details["results"][number], theme: Theme): Component {
	...
	if (isRunning && r.progress) {
		...
		c.addChild(new Text(truncLine(theme.fg("accent", `  Press ${keyText("app.tools.expand")} for live detail`), width), 0, 0));
...
export function renderSubagentResult(
	...
	if (d.mode === "single" && d.results.length === 1) {
		...
		if (isRunning && r.progress) {
			...
			c.addChild(new Text(fit(theme.fg("accent", `Press ${keyText("app.tools.expand")} for live detail`)), 0, 0));
			if (r.artifactPaths) {
