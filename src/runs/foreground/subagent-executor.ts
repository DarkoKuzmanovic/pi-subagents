...
function applyAgentDefaultContext(params: SubagentParamsLike, agents: AgentConfig[]): SubagentParamsLike {
	...
	const hasForkDefault = names.some((name) => byName.get(name)?.defaultContext === "fork");
	if (hasForkDefault) return { ...params, context: "fork" };
	const hasLineageDefault = names.some((name) => byName.get(name)?.defaultContext === "lineage");
	return hasLineageDefault ? { ...params, context: "lineage" } : params;
}
	...
	const execute = async (
		...
		if (params.action) {
			...
			if (params.action === "status") {
				...
				return inspectSubagentStatus(paramsWithResolvedCwd as import("../background/run-status.ts").RunStatusParams);
			}
			...
			return handleManagementAction(params.action, paramsWithResolvedCwd as import("../../agents/agent-management.ts").ManagementParams, { ...ctx, cwd: requestCwd });
		}
		...
		try {
			sessionFileForIndex = createSubagentContextResolver(ctx.sessionManager, effectiveParams.context, {
				cwd: effectiveCwd,
				sessionDirForIndex,
			}).sessionFileForIndex;
		} catch (error) {
		...
		const onUpdateWithContext = onUpdate
			? (r: AgentToolResult<Details>) => onUpdate(withContextDetails(r, effectiveParams.context))
			: undefined;
