...
/**
 * Remove any prior capture file so a fresh attempt cannot read a stale capture
 * written by an earlier (failed) model-candidate attempt. Best-effort.
 */
export function resetStructuredOutputCapture(runtime: StructuredOutputRuntime | undefined): void {
	if (!runtime) return;
	try {
		fs.rmSync(runtime.outputPath, { force: true });
	} catch {
		// Best-effort reset.
	}
}
