...
`minimax/MiniMax-M2.7-highspeed`, `crofai/kimi-k2.6`, `mimo/mimo-v2.5-pro`, `crofai/glm-5.1-precision`, `openai-codex/gpt-5.5`.
...
subagent({
  chain: [
    { parallel: [
        { agent: "scout",      model: "minimax/MiniMax-M2.7-highspeed", output: "recon/lane-1-localcode.md",  task: "Lane: local code. <angle>. ≤20 lines, exact file:line citations, no preamble." },
        { agent: "researcher", model: "crofai/kimi-k2.6",               output: "recon/lane-2-official.md",   task: "Lane: official sources. <angle>. ≤20 lines, source links, confidence + gaps." },
        { agent: "researcher", model: "mimo/mimo-v2.5-pro",             output: "recon/lane-3-practice.md",   task: "Lane: ecosystem/practice. <angle>. ≤20 lines, source links, confidence + gaps." }
    ] },
    { agent: "synthesizer", model: "crofai/glm-5.1-precision", output: "recon/synthesis.md",
      task: "Read recon/lane-*.md. Fuse into one decision-ready synthesis: verdict, consensus, conflicts (surface, don't smooth), key evidence with file:line/URLs preserved, gaps. Original question: {task}" }
  ],
  context: "fresh"
})
```
...
- Use the **strongest** available model for the synthesizer (synthesis is harder than recon); a cheap model produces mush.
- Do **not** ask any lane to edit files — recon is read-only unless I explicitly ask for implementation.

## 3. After the chain returns

You receive the synthesizer's compact summary + `recon/synthesis.md`. Read the synthesis. **Only** open a raw `recon/lane-*.md` brief when a decision hinges on a precise fact (exact config, file:line, version) — synthesis is great for breadth, but read the source for load-bearing specifics. If the synthesis flags an unresolved conflict, resolve it (verify the source yourself or ask me) rather than picking arbitrarily.
