...
- **Rolling no-progress thinking floods.** A child that streams past 30 MB of raw stdout since its most recent text/tool activity is aborted. Earlier successful turns no longer disable this guard for the rest of the run, and stale text in a re-serialized `message_update` snapshot does not count as fresh progress.
- **Hard cap.** A backstop bounds total output even when the stream shows progress. It counts *delta-aware* bytes—the payload each `message_update` actually adds, not the full re-serialized snapshot—so verbose-but-honest runs are not charged quadratic snapshot amplification toward the 200 MB model-output cap. A separate 1 GB raw-byte backstop bounds other flood shapes. Size-triggered errors report raw bytes, accounted bytes, amplification ratio, and bytes since meaningful progress.
