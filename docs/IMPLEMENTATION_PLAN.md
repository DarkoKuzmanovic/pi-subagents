...
- [x] **Decide async-by-default routing for chains (`c567660`)** — restore the pre-`c567660` chain-specific gate: `requestedAsync && (hasChain ? clarify === false : clarify !== true)`. The merge collapsed chains into the single/parallel rule, so user config `asyncByDefault: true` silently bypasses the documented chain-default clarify UI before `executeChain` can apply `clarify !== false`. Existing contracts agree that chains require `clarify: false` for background execution; `forceTopLevelAsync` remains authoritative because it explicitly rewrites to `{ async: true, clarify: false }`. No separate config is justified. Decision: 2026-07-14.
- [ ] **Implement the chain-routing restoration as v0.42.2** — add a routing truth-table regression covering chain/single/parallel with omitted and explicit `async`/`clarify`, preserve foreground-without-TUI behavior for parallel-containing chains, then restore only the chain branch of `effectiveAsync`. Explicit chain `async: true` continues to require `clarify: false`, matching the current README and schema contract.
...
- [x] **Async runner-crash + hollow-salvage hardening** — root-caused the recurring "runner exited before writing result" failures to a `ReferenceError: tokenUsageFromAttempts is not defined` in the parallel token-accounting path (defined the helper + wrapped the block in try/catch, 0.34.2); and made stale-run reconciliation recover per-step output from `output-<i>.log` instead of salvaging empty (0.34.3). Confirmed via identical stderr traces across runs `8d0ce5b7`/`deb83f54`/`2a652574`.
- [x] **Completion-guard false-positives** — embedded-payload stripping + analysis-only intent patterns so read-only/analysis tasks with implementation keywords in their data aren't flagged (0.34.1); and tool-aware exemption so agents without `edit`/`write` (oracle/oracle-fresh, any read-only agent) are never expected to mutate (0.34.4).
